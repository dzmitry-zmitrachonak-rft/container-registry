...
-- => Those operations have to get serialized
...
-- find all dangling blob candidates: index digest where layer_id is null
-- find dangling blobs: select digest from blobs_layers group by digest having every (layer_id is null)
-- check if blob is used: select 1 from blobs_layers where digest = ? AND layer_id is not null limit 1
CREATE TABLE public.blobs_layers (
  ...
  FOREIGN KEY (repository_id, layer_id) REFERENCES layers (repository_id, id) ON DELETE CASCADE -- SET NULL
) PARTITION BY HASH (digest);

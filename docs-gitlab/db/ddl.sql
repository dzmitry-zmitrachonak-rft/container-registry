...
CREATE INDEX index_repositories_on_namespace_id ON repositories (namespace_id);

CREATE INDEX index_repositories_on_namespace_id_and_parent_id ON repositories (namespace_id, parent_id);
...
CREATE INDEX index_repository_blobs_on_namespace_id_and_repository_id ON repository_blobs (namespace_id, repository_id);
...
CREATE TABLE manifests (
    ...
    CONSTRAINT unique_manifests_repository_id_and_digest UNIQUE (namespace_id, repository_id, digest), -- do we really need `namespace_id` here (no FKs for this trio)?
...
CREATE INDEX index_manifests_on_namespace_id_and_repository_id ON manifests (namespace_id, repository_id);
...
CREATE TABLE manifest_references (
    ...
    CONSTRAINT unique_manifest_references_nmspc_id_rpsty_id_prnt_id_chld_id UNIQUE (namespace_id, repository_id, parent_id, child_id), -- do we really need `namespace_id` here?
    CONSTRAINT check_manifest_references_parent_id_and_child_id_differ CHECK (parent_id <> child_id)
)
PARTITION BY HASH (namespace_id);

CREATE INDEX index_manifest_references_on_nmspc_id_and_rpstry_id_and_prnt_id ON manifest_references (namespace_id, repository_id, parent_id);

CREATE INDEX index_manifest_references_on_nmspc_id_and_rpstry_id_and_chld_id ON manifest_references (namespace_id, repository_id, child_id);
...
CREATE INDEX index_layers_on_namespace_id_and_repository_id_and_manifest_id ON layers (namespace_id, repository_id, manifest_id);
...
CREATE TABLE tags (
    ...
    CONSTRAINT unique_tags_repository_id_and_name UNIQUE (namespace_id, repository_id, name), -- do we really need `namespace_id` here?
    CONSTRAINT check_tags_name_length CHECK ((char_length(name) <= 255))
)
PARTITION BY HASH (namespace_id);

CREATE INDEX index_tags_on_namespace_id_and_repository_id_and_manifest_id ON tags (namespace_id, repository_id, manifest_id);

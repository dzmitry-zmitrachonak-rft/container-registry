...
func init() {
	m := &migrate.Migration{
		...
		Up: []string{
			...
			"CREATE INDEX IF NOT EXISTS ix_manifests_repository_id ON manifests USING btree (repository_id)",
			"CREATE INDEX IF NOT EXISTS ix_manifests_media_type_id ON manifests USING btree (media_type_id)",
			"CREATE INDEX IF NOT EXISTS ix_manifests_configuration_media_type_id ON manifests USING btree (configuration_media_type_id)",
			"CREATE INDEX IF NOT EXISTS ix_manifests_configuration_blob_digest ON manifests USING btree (configuration_blob_digest)",
		},
		Down: []string{
			"DROP INDEX IF EXISTS ix_manifests_configuration_blob_digest CASCADE",
			"DROP INDEX IF EXISTS ix_manifests_configuration_media_type_id CASCADE",
			"DROP INDEX IF EXISTS ix_manifests_media_type_id CASCADE",
			"DROP INDEX IF EXISTS ix_manifests_repository_id CASCADE",

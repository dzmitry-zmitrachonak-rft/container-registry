...
func reloadConfigurationFixtures(tb testing.TB) {
	testutil.ReloadFixtures(tb, suite.db, suite.basePath, testutil.BlobsTable, testutil.ConfigurationsTable,
		testutil.ManifestsTable)
}

func unloadConfigurationFixtures(tb testing.TB) {
	require.NoError(tb, testutil.TruncateTables(suite.db, testutil.BlobsTable, testutil.ConfigurationsTable,
		testutil.ManifestsTable))
}
...
func TestConfigurationStore_Manifests(t *testing.T) {
	reloadConfigurationFixtures(t)

	s := datastore.NewConfigurationStore(suite.db)
	mm, err := s.Manifests(suite.ctx, &models.Configuration{ID: 1})
	require.NoError(t, err)
